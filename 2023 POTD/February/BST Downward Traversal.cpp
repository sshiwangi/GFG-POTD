...
public:
    ...
    long long int verticallyDownBST(Node *root,int target){
        ...
        q.push({target_location,0});// we are storing target at 0 index
        if(target_location == NULL)return -1;
        int ans = 0; // storing the sum of all nodes lying down the target node
        while(!q.empty()){
            Node* curr = q.front().first;
            int currpos = q.front().second;
            q.pop();
            if(currpos == 0){// if the node lie down the currpos
                ans += curr->data;
            }
            if(curr->left)q.push({curr->left,currpos-1});
            if(curr->right)q.push({curr->right,currpos+1});
        }
        return ans-target;// because we stored target's value in the queue so we need to remove it from our answer
    }
};
